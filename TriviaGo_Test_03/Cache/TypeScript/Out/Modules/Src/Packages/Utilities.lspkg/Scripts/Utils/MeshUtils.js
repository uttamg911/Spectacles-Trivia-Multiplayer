...
class MeshUtils {
     ...
     */
    static makeCircle2DMesh(position, radius) {
     ...
     */
    static makeCircle2DIndicesVerticesPair(position, radius, segments, indicesOffset) {
        const indices = [];
        const vertices = [];
     ...
     * @returns Generated render mesh
     */
    static makeLineStrip2DMeshWithJoints(positions, thickness) {
        const builder = new MeshBuilder([{ name: "position", components: 3 }]);
        builder.topology = MeshTopology.Triangles;
        builder.indexType = MeshIndexType.UInt16;
        for (let i = 0; i < positions.length - 1; ++i) {
            const [indices, vertices] = MeshUtils.makeLine2DIndicesVerticesPair(positions[i], positions[i + 1], thickness, i * 4);
            builder.appendIndices(indices);
            builder.appendVerticesInterleaved(vertices);
        }
        const segments = 16;
        ...
        for (let i = 0; i < positions.length; ++i) {
            const [indices, vertices] = MeshUtils.makeCircle2DIndicesVerticesPair(positions[i], radius, segments, linesIndicesOffset + i * segments);
            builder.appendIndices(indices);
            builder.appendVerticesInterleaved(vertices);
        }
        builder.updateMesh();
     ...
     * @returns [indices, vertices] arrays
     */
    static makeLine2DIndicesVerticesPair(start, end, thickness, indicesOffset) {
     ...
     */
    static addRenderMeshVisual(sceneObject, mesh, material, renderOrder) {
