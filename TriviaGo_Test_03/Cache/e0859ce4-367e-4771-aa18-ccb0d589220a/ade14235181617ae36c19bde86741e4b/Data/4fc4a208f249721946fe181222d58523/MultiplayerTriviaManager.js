 ...
 *
 * PROPERLY SYNCED MULTIPLAYER BEHAVIOR
 */
...
let MultiplayerTriviaManager = (() => {
    ...
    var MultiplayerTriviaManager = _classThis = class extends _classSuper {
        constructor() {
            ...
            this.hostPlayerEntity = null;
            this.guestPlayerEntity = null;
        ...
        onSessionReady() {
            ...
            // CRITICAL: Listen to BOTH buzzer timestamps - freeze timer immediately when ANY arrive
            ...
            // CRITICAL: Only host should listen to ready flags
            this.hostReadyProp.onAnyChange.add(() => {
                if (this.isHost) {
                    this.log('hostReadyProp changed: ' + this.hostReadyProp.currentValue);
                    this.evaluateHostQuestionFetch();
                }
            });
            this.guestReadyProp.onAnyChange.add(() => {
                if (this.isHost) {
                    this.log('guestReadyProp changed: ' + this.guestReadyProp.currentValue);
                    this.evaluateHostQuestionFetch();
                }
            });
        ...
        handleRoundStateChange() {
            ...
            if (state === RoundPhase.PLAYING) {
                this.log('Round phase: PLAYING');
            }
            else if (state === RoundPhase.EVALUATED) {
                this.log('Round phase: EVALUATED (waiting for reveal)');
            }
        ...
        setupProcessButton() {
            ...
            this.processButton.onTriggerUp.add(() => {
                this.log('Process button pressed by ' + (this.isHost ? 'HOST' : 'GUEST'));
                this.hideAnswerFeedback();
                this.localHasAnsweredPhase = false;
                this.timerActive = false;
                this.localTimeRemaining = 0;
                ...
                // CRITICAL: Each player clears ONLY their own buzzer token
                if (this.isHost) {
                    this.log('Host: clearing hostBuzzedTime and setting hostReady=true');
                    this.hostBuzzedTimeProp.setPendingValue('');
                    this.hostReadyProp.setPendingValue(true);
                }
                else {
                    this.log('Guest: clearing guestBuzzedTime and setting guestReady=true');
        ...
        evaluateHostQuestionFetch() {
            ...
            this.log(`evaluateHostQuestionFetch: hostReady=${hostReady}, guestReady=${guestReady}`);
            const session = SessionController_1.SessionController.getInstance();
            ...
            if (roomIsReady) {
                this.log('Room is ready! Fetching new question...');
                this.hostReadyProp.setPendingValue(false);
                this.guestReadyProp.setPendingValue(false);
                this.hostBuzzedTimeProp.setPendingValue('');
                // Do NOT clear guestBuzzedTimeProp here - only guest can clear it
        ...
        fetchAndSync() {
            ...
            this.internetModule.performHttpRequest(request, (response) => {
                if (response.statusCode !== 200) {
                    ...
                    this.log('Error loading question: ' + response.statusCode);
                    return;
                }
                try {
                    const result = JSON.parse(response.body);
                    if (result.ok === true && result.record) {
                        this.log('Question loaded successfully');
