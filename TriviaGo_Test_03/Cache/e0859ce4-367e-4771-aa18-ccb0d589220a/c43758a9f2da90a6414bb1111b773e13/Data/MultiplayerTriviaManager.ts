 ...
 * - Cleared hostBuzzedTimeProp and guestBuzzedTimeProp by setting them to an empty string ('')
 * inside evaluateHostQuestionFetch to prevent stale player input from instantly breaking 
 * the authoritative turn processor on subsequent rounds.
 */
...
export class MultiplayerTriviaManager extends BaseScriptComponent {
  ...
  private evaluateHostQuestionFetch() {
    ...
    if (roomIsReady) {
      ...
      // FIX HERE: Re-initialize the buzz properties back to empty strings 
      // so the authoritative loop doesn't read dirty data from the prior round.
      this.hostBuzzedTimeProp.setPendingValue('')
      this.guestBuzzedTimeProp.setPendingValue('')
