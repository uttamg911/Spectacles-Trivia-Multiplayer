 ...
 * - Stops the local timer instantly for whichever player clicks first.
 * - Eliminates the "waiting for verification" delay.
 * - Host remains the authoritative referee for global scoring over the network.
 */
...
let MultiplayerTriviaManager = (() => {
    ...
    var MultiplayerTriviaManager = _classThis = class extends _classSuper {
        ...
        handleRoundStateChange() {
            ...
            // If an official network state arrives, make sure our local UI reflects it
        ...
        evaluateBuzzerRaceAuthoritative() {
            ...
            const activeBuzzer = this.currentActiveBuzzerProp.currentValue;
            // ── SCENARIO A: The initial open race phase
            if (activeBuzzer === 'NONE') {
        ...
        // ───────────────────────────────────────────────────────────────────────────
        onUpdate() {
            // 1. Authoritative Host polling check
            if (this.isHost && this.roundStateProp.currentValue === 'PLAYING') {
                this.evaluateBuzzerRaceAuthoritative();
            }
            // 2. Local Timer Counting Loop
            if (!this.timerActive)
                return;
        ...
        checkUserAnswer(index) {
            ...
            // ─── CLIENT-SIDE PREDICTION (Instant UI Reaction) ────────────────────────
            this.timerActive = false; // Stop the timer locally right now!
            this.hideAnswerFeedback();
            const isCorrect = index === this.correctAnswer;
            if (isCorrect) {
                if (this.correctText)
                    this.correctText.enabled = true;
                this.setStatusText("Correct! Sending to server...");
            }
            else {
                if (this.incorrectText)
                    this.incorrectText.enabled = true;
                // If it's a miss during open race mode, prepare for a steal turn locally
            ...
            // ─────────────────────────────────────────────────────────────────────────
            // Dispatch payload to network authority channel
