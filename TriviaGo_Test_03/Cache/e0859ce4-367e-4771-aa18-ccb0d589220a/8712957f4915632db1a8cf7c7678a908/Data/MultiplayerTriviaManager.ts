 ...
 *
 * FIXES in this version (2 precise changes, nothing else touched):
 *
 * FIX 1 — Score listeners registered too early (scores never updated):
 *   setupScoreboardCrossTalk() was called before the SyncEntities were
 *   constructed. StorageProperty.onAnyChange only fires once the property
 *   is wired to a live SyncEntity. Moved the call to immediately after
 *   both SyncEntities are built.
 *
 * FIX 2 — Host timer not freezing (network propagation race):
 *   The host timer could only freeze inside processTurnAuthoritative, which
 *   runs only after evaluateBuzzerRaceAuthoritative sees the buzz token.
 *   But the token (guestBuzzedTimeProp / hostBuzzedTimeProp) takes 1-3 frames
 *   to propagate from the guest device to the host. During those frames
 *   onUpdate kept ticking. Fix: freeze timerActive=false in the onAnyChange
 *   callbacks for BOTH buzz token props. The instant any token arrives on any
 *   device, the timer stops — before the turn is even evaluated.
 */
...
export class MultiplayerTriviaManager extends BaseScriptComponent {
  ...
  private playerSyncEntity: SyncEntity | null = null

  // gameSyncEntity props — host owns and writes all of these
  ...
  private onSessionReady() {
    ...
    // ── Build SyncEntities FIRST ─────────────────────────────────────────────
    // onAnyChange listeners only fire once properties are wired to a live entity.
    // Everything that calls onAnyChange.add() must happen AFTER these two blocks.
    ...
    this.playerSyncEntity = new SyncEntity(
      this,
      new StoragePropertySet([
        this.hostReadyProp,
    ...
    // ── FIX 1: attach ALL onAnyChange listeners AFTER entities are built ─────
    // Previously setupScoreboardCrossTalk() was called before SyncEntities were
    // constructed, so its onAnyChange listeners were never triggered.
    ...
    // Timer start — resets the round clock and clears the turn guard
    ...
    // FIX 2 — freeze timer the instant ANY buzz token arrives on this device.
    // This eliminates the 1-3 frame propagation window where the host's timer
    // kept running because evaluateBuzzerRace hadn't seen the token yet.
    this.hostBuzzedTimeProp.onAnyChange.add(() => {
      if (this.hostBuzzedTimeProp.currentValue) {
        this.timerActive = false
        this.log('Timer frozen: hostBuzzedTime received')
      }
    })
    this.guestBuzzedTimeProp.onAnyChange.add(() => {
      if (this.guestBuzzedTimeProp.currentValue) {
        this.timerActive = false
        this.log('Timer frozen: guestBuzzedTime received')
      }
    })
  ...
  private processTurnAuthoritative(player: 'HOST' | 'GUEST', chosenOption: number) {
    // Belt-and-suspenders: also freeze here in case the onAnyChange path was missed
    this.timerActive = false
    ...
    if (isCorrect) {
      ...
      if (activeBuzzer === 'NONE') {
        ...
        this.turnProcessed = false  // allow steal-window evaluation
  ...
  private setupScoreboardCrossTalk() {
    ...
    this.guestScoreProp.onAnyChange.add(() => {
      ...
      this.log(`guestScore changed → ${score}`)
      if (!this.isHost) {
        this.localScore = score
        if (this.myScoreValueText) this.myScoreValueText.text = String(score)
      } else {
  ...
  private checkUserAnswer(index: number) {
    ...
    // Local prediction — freeze UI on this device immediately
    this.timerActive = false
    this.hideAnswerFeedback()
    ...
    // Send buzz token — the onAnyChange on the OTHER device freezes their timer
