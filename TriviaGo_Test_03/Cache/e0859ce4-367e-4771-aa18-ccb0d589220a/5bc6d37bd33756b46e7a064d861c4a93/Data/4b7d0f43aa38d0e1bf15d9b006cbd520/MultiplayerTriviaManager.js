 ...
 *
 * FIX APPLIED:
 * Prevented host from attempting unauthorized network writes to guest-owned entities.
 * Fixed "ghost tokens" carrying over from previous rounds by moving property clearing
 * directly to the respective entity owners during initialization and round transitions.
 */
    ...
    var MultiplayerTriviaManager = _classThis = class extends _classSuper {
        ...
        processTurnAuthoritative(player, chosenOption) {
            ...
            if (isCorrect) {
                ...
                if (activeBuzzer === 'NONE') {
                    // FIX: The host clears ONLY what it owns.
        ...
        setupProcessButton() {
            ...
            this.processButton.onTriggerUp.add(() => {
                ...
                // FIX: Clean local sync variables upon moving to the next round 
                // directly inside the owning context.
                if (this.isHost) {
                    this.hostBuzzedTimeProp.setPendingValue('');
        ...
        evaluateHostQuestionFetch() {
            ...
            if (roomIsReady) {
                ...
                this.hostBuzzedTimeProp.setPendingValue('');
                // FIX: Left guest token clearing handled on the guest's side via setupProcessButton
