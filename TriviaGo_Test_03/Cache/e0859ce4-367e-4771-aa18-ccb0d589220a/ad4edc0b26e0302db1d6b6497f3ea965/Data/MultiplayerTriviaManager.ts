 ...
 *
 * Full Production Script - Fixed Shared Real-Time Synchronization and Remote Fetch Invocation.
 */
...
export class MultiplayerTriviaManager extends BaseScriptComponent {
  ...
  // ── Synced Network Properties (Global Shared Network Storage Block) ────────
  ...
  private onSessionReady() {
    ...
    // FIX: Rebuilt utilizing a shared authority framework topology mapping.
    // Both peers connect to the exact same network component mapping room.
    const gameNetworkId = { customNetworkId: 'triviaGlobalUnifiedStateBlock', networkIdType: 'Custom' } as any
    ...
    // Question updating network sync hooks
    ...
    // Track dynamic changes to room ready arrays across devices
    ...
    this.gameSyncEntity.notifyOnReady(() => {
      this.log('Network sync entity system operational.')
  ...
  private setupScoreboardCrossTalk() {
    ...
    this.guestScoreProp.onAnyChange.add(() => {
      ...
      this.log(`Guest score changed network value: ${score}`)
      if (!this.isHost) {
        if (this.myScoreValueText) this.myScoreValueText.text = String(score)
      } else {
        if (this.opponentScoreValueText) this.opponentScoreValueText.text = String(score)
  ...
  private evaluateHostQuestionFetch() {
    // Only the host runs the data ingestion script block
    ...
    // Process immediately if playing solo; otherwise, wait until both profiles register true
    const roomIsReady = hasRemoteOpponent ? (hostReady && guestReady) : hostReady

    if (roomIsReady) {
      this.log('Room validated. Resetting ready states and running edge function fetch...');
  ...
  private setupProcessButton() {
    ...
    this.processButton.onTriggerUp.add(() => {
      this.log('Process button pressed. Registering local ready status across network...');
      this.hideAnswerFeedback()
  ...
  private syncScoreToNetwork() {
    this.log(`Pushing score out to network: ${this.localScore} (isHost: ${this.isHost})`)
    if (this.isHost) {
  ...
  private handleEdgeFunctionResponse(body: string) {
    try {
      ...
      // Force state properties over network layers to update matching client devices
