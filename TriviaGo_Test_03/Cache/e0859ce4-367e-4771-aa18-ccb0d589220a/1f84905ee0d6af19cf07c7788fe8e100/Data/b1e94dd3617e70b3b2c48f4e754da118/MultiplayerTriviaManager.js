 ...
 *
 * Hardened Buzzer Race Mechanics:
 * - Host uses Update-Loop Polling to check player buzzers, guaranteeing no missed network events.
 * - Player answers stop the timer and update scores identically to the Host.
 */
    ...
    var MultiplayerTriviaManager = _classThis = class extends _classSuper {
        ...
        processTurnAuthoritative(player, chosenOption) {
            ...
            if (isCorrect) {
                ...
                // If this was the first miss, pass the turn to the opponent
                if (activeBuzzer === 'NONE') {
                    // Clear tokens so old values don't re-trigger the next step instantly
        ...
        // ───────────────────────────────────────────────────────────────────────────
        onUpdate() {
            // 1. Authoritative Host checks properties every frame to process fast-clicks immediately
        ...
        evaluateHostQuestionFetch() {
            ...
            if (roomIsReady) {
                ...
                // Completely flush tokens to pristine defaults for the new question
                this.hostBuzzedTimeProp.setPendingValue('');
                this.guestBuzzedTimeProp.setPendingValue('');
