...
export class MultiplayerTriviaManager extends BaseScriptComponent {
  ...
  private onSessionReady() {
    ...
    // We only need a blank store because we will pass data instantly via custom events
    ...
    // Player receives the full question package instantly
    this.gameSyncEntity.onEventReceived.add('syncFullQuestion', (jsonPayload: string) => {
      this.log('Received raw question package from Host')
      try {
        const record = JSON.parse(jsonPayload) as TriviaRecord
        this.applyQuestionDataDirectly(record)
      } catch (e) {
        this.log(`Error parsing synced payload: ${e}`)
      }
    })
  ...
  private handleEdgeFunctionResponse(body: string) {
    try {
      ...
      // 1. Instantly update Host's local UI layout
      this.applyQuestionDataDirectly(r)

      // 2. Broadcast the entire question structure to all connected clients at the same time
      this.gameSyncEntity?.sendEvent('syncFullQuestion', JSON.stringify(r))
