 ...
 * 4. Turn Processing Guard: turnProcessed flag is now properly reset only when entering
 * a fresh PLAYING state, preventing stale token re-evaluation loops.
 */
...
export class MultiplayerTriviaManager extends BaseScriptComponent {
  ...
  private handleRoundStateChange() {
    ...
    if (state === 'PLAYING') {
      // Reset turnProcessed when entering a fresh round
      this.turnProcessed = false
  ...
  private processTurnAuthoritative(player: 'HOST' | 'GUEST', chosenOption: number) {
    ...
    if (isCorrect) {
      ...
      if (activeBuzzer === 'NONE') {
        ...
        // CRITICAL: Do NOT reset turnProcessed here — keep it true until next PLAYING state
