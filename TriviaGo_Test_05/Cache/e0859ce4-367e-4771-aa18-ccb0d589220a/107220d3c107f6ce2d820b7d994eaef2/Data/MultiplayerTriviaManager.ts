 ...
 * Merging hostReady/guestReady/hostBuzzedTime/guestBuzzedTime into a single
 * playerSyncEntity with persistent=true does NOT grant both players write access.
 * In SpectaclesSyncKit, whoever constructs the entity first becomes its owner.
 * The non-owner's setPendingValue calls are silently dropped — no network write
 * occurs, so onAnyChange never fires on the other device.
 *
 * This caused both symptoms:
 *   - Host timer never froze: guestBuzzedTimeProp write was dropped, so the
 *     onAnyChange that freezes the timer never fired on the host.
 *   - Scores never updated: evaluateBuzzerRaceAuthoritative never saw a buzz
 *     token, so processTurnAuthoritative (which writes scores) never ran.
 *
 * FIX: Restore the two-entity split for player-written props:
 *   hostPlayerEntity  — owned by host  — holds hostReady + hostBuzzedTime
 *   guestPlayerEntity — owned by guest — holds guestReady + guestBuzzedTime
 * Each player owns the entity they write to. Both players can read both entities.
 * Scores remain in gameSyncEntity (host-owned), which was already correct.
 */
...
export class MultiplayerTriviaManager extends BaseScriptComponent {
  ...
  // ── SyncEntities ────────────────────────────────────────────────────────────
  private gameSyncEntity:   SyncEntity | null = null  // host-owned: all game state + scores
  private hostPlayerEntity: SyncEntity | null = null  // host-owned: host writes here
  private guestPlayerEntity: SyncEntity | null = null // guest-owned: guest writes here
  ...
  private onSessionReady() {
    ...
    // ── 1. gameSyncEntity: host-owned, carries all authoritative game state ───
    // Host is the only writer. Both players receive changes via onAnyChange.
    ...
    // ── 2. hostPlayerEntity: owned by host, host writes ready + buzz token ────
    // isHost=true means the host is the owner and sole writer.
    // The guest device constructs this entity too (required to subscribe) but
    // cannot write to it — which is exactly what we want.
    ...
    // ── 3. guestPlayerEntity: owned by guest, guest writes ready + buzz token ─
    // !isHost means the guest is the owner and sole writer of this entity.
    // The host device constructs it to subscribe, but cannot write to it.
    ...
    // ── All onAnyChange listeners registered AFTER all entities are built ─────
    // Listeners only fire once the property is wired to a live SyncEntity.
    ...
    // Freeze timer the instant a buzz token arrives on this device.
    // Because each buzz prop lives in its owner's entity, writes now propagate
    // correctly — so these onAnyChange callbacks will actually fire.
  ...
  private processTurnAuthoritative(player: 'HOST' | 'GUEST', chosenOption: number) {
    ...
    if (isCorrect) {
      ...
      if (activeBuzzer === 'NONE') {
        // Clear tokens so steal window starts clean, then open steal for opponent
  ...
  // ───────────────────────────────────────────────────────────────────────────
  // Scoreboard — registered after SyncEntities are built so onAnyChange fires
  // ───────────────────────────────────────────────────────────────────────────
  ...
  private checkUserAnswer(index: number) {
    ...
    // Write to the entity this player OWNS — this is what makes the write succeed
  ...
  private setupProcessButton() {
    ...
    this.processButton.onTriggerUp.add(() => {
      ...
      // Write to the entity this player owns
      if (this.isHost) {
