 ...
 *
 * Fully reactive multiplayer trivia state controller.
 */
...
export class MultiplayerTriviaManager extends BaseScriptComponent {
  ...
  // ── NEW INPUTS: Scoreboard, Timer & Ready States ───────────────────────────
  ...
  private opponentScorePropRef: StorageProperty<number> | null = null
  private opponentReadyPropRef: StorageProperty<boolean> | null = null
  ...
  onAwake() {
    ...
    this.createUpdateEvent('onUpdateEvent')
  }

  private onSessionReady() {
    this.log('Session connection established')
    const session = SessionController.getInstance()
    this.localPlayerId = session.getLocalUserInfo()?.connectionId ?? 'localPlayer'
    ...
    session.onUserJoinedSession.add((userInfo) => {
      if (userInfo.connectionId !== this.localPlayerId) {
        this.hookOpponentStore(userInfo.connectionId)
      }
    })

    // Audit active canvas users in case opponent landed before initialization loop completed
    const currentUsers = session.getActiveUsersInfo()
    for (let i = 0; i < currentUsers.length; i++) {
      if (currentUsers[i].connectionId !== this.localPlayerId) {
        this.hookOpponentStore(currentUsers[i].connectionId)
        break
      }
    }
  ...
  private createReadyCheckHooks() {
    // Whenever our local state flips, evaluating requirements if we hold the host seat
    this.myReadyProp.onAnyChange.add(() => this.evaluateHostQuestionFetch())

    // Safe interval evaluation block capturing late network packets
    Script.createRetryingTimer(() => {
      if (this.opponentReadyPropRef) {
        this.opponentReadyPropRef.onAnyChange.add(() => this.evaluateHostQuestionFetch())
        return true // Cancel retention timer once reference hooks activate
      }
      return false
    }, 0.2)
  }
  ...
  private evaluateHostQuestionFetch() {
    ...
    // If there is an opponent, both must be ready. Otherwise, host can test solo.
    ...
    if (roomIsReady) {
      ...
      // Automatically toggle flags off before executing fetch pipeline to prevent cyclical calls
  ...
  // ───────────────────────────────────────────────────────────────────────────
  // High Precision Synchronized Local Timer Event Loop (Feature 2)
  // ───────────────────────────────────────────────────────────────────────────
  ...
  onUpdateEvent() {
    ...
    const currentTime = SessionController.getInstance().getServerTimeInSeconds()
  ...
  private onRoundTimerExpired() {
    ...
    if (!this.correctAnswerSelected && this.doOnce) {
      // Treat remaining un-submitted timers as incorrect timeouts
  ...
  private checkUserAnswer(index: number) {
    // Lock response submission window if current system server time surpasses deadline
    const currentTime = SessionController.getInstance().getServerTimeInSeconds()
    const targetEndTime = this.timerEndServerTimeProp.currentValue ?? 0
  ...
  private handleEdgeFunctionResponse(body: string) {
    try {
      ...
      // Calculate countdown configuration via network timeline parameters
      const currentServerTime = SessionController.getInstance().getServerTimeInSeconds()
      const endServerTimestamp = currentServerTime + this.roundDurationSeconds

      // Lock sequence properties values safely back onto cloud storage arrays
      this.timerEndServerTimeProp.setPendingValue(endServerTimestamp)
      this.jsonQuestionProp.setPendingValue(JSON.stringify(r))

      // Update local Host interface configuration instantly
