 ...
 *
 * Fixed, fully-compiled multiplayer trivia state controller.
 */
...
export class MultiplayerTriviaManager extends BaseScriptComponent {
  ...
  // Track the mirrored opponent properties dynamically (typed as any to prevent strict type failures)
  ...
  private onSessionReady() {
    ...
    session.onUserJoinedSession.add((userInfo) => {
      if (userInfo && userInfo.connectionId !== this.localPlayerId) {
        this.hookOpponentStore(userInfo.connectionId)
      }
    })

    // Audit active session participants safely using standard user loops
    const currentUsers = session.getActiveUsersInfo()
    if (currentUsers) {
      for (let i = 0; i < currentUsers.length; i++) {
        const u = currentUsers[i]
        if (u && u.connectionId !== this.localPlayerId) {
          this.hookOpponentStore(u.connectionId)
          break
        }
      }
    }
  ...
  private createReadyCheckHooks() {
    ...
    // Standard JavaScript interval loop to await remote tracking reference loads safely
