 ...
 * 1. Ownership Compliance: Host only writes to properties it owns
 * 2. Guard against re-evaluation: Use roundState change, not turnProcessed pre-check
 * 3. Proper evaluation timing: Ensure fresh tokens are read before evaluation
 */
...
export class MultiplayerTriviaManager extends BaseScriptComponent {
  ...
  // ── SyncEntities ────────────────────────────────────────────────────────────
  private gameSyncEntity:   SyncEntity | null = null  // host-owned: game state + scores
  ...
  private lastProcessedHostToken: string = ''
  private lastProcessedGuestToken: string = ''
  ...
  private onSessionReady() {
    ...
    this.timerStartTokenProp.onAnyChange.add(() => {
      ...
      if (duration > 0) {
        ...
        this.lastProcessedHostToken = ''
        this.lastProcessedGuestToken = ''
  ...
  private handleRoundStateChange() {
    ...
    if (state === 'PLAYING') {
      // Reset token tracking when entering a fresh round
      this.lastProcessedHostToken = ''
      this.lastProcessedGuestToken = ''
    } else if (state === 'REVEAL_CORRECT') {
      this.timerActive = false
      this.hideAnswerFeedback()
  ...
  private evaluateBuzzerRaceAuthoritative() {
    ...
    if (activeBuzzer === 'NONE') {
      // Race condition: evaluate only if we have NEW tokens (not seen before)
      if (!hostToken && !guestToken) return

      const hostIsNew  = hostToken && hostToken !== this.lastProcessedHostToken
      const guestIsNew = guestToken && guestToken !== this.lastProcessedGuestToken

      if (!hostIsNew && !guestIsNew) return
      ...
      this.lastProcessedHostToken = hostToken
      this.lastProcessedGuestToken = guestToken
  ...
  private processTurnAuthoritative(player: 'HOST' | 'GUEST', chosenOption: number) {
    ...
    if (isCorrect) {
      ...
      if (activeBuzzer === 'NONE') {
        // The host clears ONLY the host property it owns
  ...
  private setupProcessButton() {
    ...
    this.processButton.onTriggerUp.add(() => {
      ...
      // Clean local network sync values cleanly upon round transition 
      if (this.isHost) {
        this.hostBuzzedTimeProp.setPendingValue('')
