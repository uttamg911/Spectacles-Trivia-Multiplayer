...
export class MultiplayerTriviaManager extends BaseScriptComponent {
  ...
  private hasBuzzedThisRound: boolean   = false
  ...
  private checkUserAnswer(index: number) {
    if (!this.timerActive || this.hasBuzzedThisRound) return
    ...
    if (currentBuzzerState === 'PASSED_TO_GUEST' && this.isHost) return
    if (currentBuzzerState === 'PASSED_TO_HOST' && !this.isHost) return
