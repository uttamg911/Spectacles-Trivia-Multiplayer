 ...
 *
 * Fixed Score Replication, Ready Synchronicity, and Cross-Device Validation.
 */
...
let MultiplayerTriviaManager = (() => {
    ...
    var MultiplayerTriviaManager = _classThis = class extends _classSuper {
        ...
        onSessionReady() {
            ...
            // 1. Shared Global Room Configuration (Questions and Timers)
            const gameNetworkId = { customNetworkId: 'triviaGlobalUnifiedState', networkIdType: 'Custom' };
            this.gameSyncEntity = new SyncEntity_1.SyncEntity(this, new StoragePropertySet_1.StoragePropertySet([this.jsonQuestionProp, this.timerStartTokenProp]), true, 'Session', gameNetworkId);
            // 2. Separate Write Authority Channels to Guarantee Bi-Directional Score Syncs
            ...
            this.hostDataSyncEntity = new SyncEntity_1.SyncEntity(this, new StoragePropertySet_1.StoragePropertySet([this.hostScoreProp, this.hostReadyProp]), this.isHost, 'Session', hostNetworkId);
            this.guestDataSyncEntity = new SyncEntity_1.SyncEntity(this, new StoragePropertySet_1.StoragePropertySet([this.guestScoreProp, this.guestReadyProp]), !this.isHost, 'Session', guestNetworkId);
            this.setupScoreboardCrossTalk();
            // Process questions received across network infrastructure
            this.jsonQuestionProp.onAnyChange.add(() => {
                const rawJson = this.jsonQuestionProp.currentValue;
                if (rawJson)
                    this.parseAndApplyJson(rawJson);
            });
            // Read the time duration payload
            ...
            // Both players observe modifications; the Host logic handles the fetch
        ...
        evaluateHostQuestionFetch() {
            // Only the host can execute the database request
            if (!this.isHost)
                return;
            ...
            if (roomIsReady) {
                this.log('Room validated. Resetting ready markers and updating network payloads…');
