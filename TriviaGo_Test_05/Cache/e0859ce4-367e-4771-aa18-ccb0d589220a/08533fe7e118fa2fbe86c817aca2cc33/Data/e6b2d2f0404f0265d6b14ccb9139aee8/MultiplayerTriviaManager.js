 ...
 *
 * Fixed Buzzer Race Network Bubbling (TypeScript Type Compilation Fix):
 * - Fixed the onAnyChange callback signature to map directly to the string argument.
...
let MultiplayerTriviaManager = (() => {
    ...
    var MultiplayerTriviaManager = _classThis = class extends _classSuper {
        ...
        onSessionReady() {
            ...
            // Fixed callback definitions: Sync Kit passes the new string property value directly to the listener
            if (this.isHost) {
                this.hostBuzzedTimeProp.onAnyChange.add((newHostToken) => {
                    this.evaluateBuzzerRace(newHostToken, this.guestBuzzedTimeProp.currentValue);
                });
                this.guestBuzzedTimeProp.onAnyChange.add((newGuestToken) => {
                    this.evaluateBuzzerRace(this.hostBuzzedTimeProp.currentValue, newGuestToken);
                });
            }
        ...
        evaluateBuzzerRace(hostToken, guestToken) {
            if (!this.isHost)
                return;
