 ...
 * 2. State-based Turn Guard: Use roundState transitions to prevent stale evaluations
 * 3. Proper Sync Timing: Wait for fresh round state before accepting new buzzer tokens
 */
...
export class MultiplayerTriviaManager extends BaseScriptComponent {
  ...
  private evaluateBuzzerRaceAuthoritative() {
    ...
    // CRITICAL: Only evaluate during PLAYING phase. Once we move to EVALUATED,
    // we wait for the reveal states, preventing re-evaluation of stale tokens.
    ...
    if (activeBuzzer === 'NONE') {
      // Race condition: only evaluate if at least one player has buzzed
      ...
      // Move to EVALUATED state immediately to prevent re-evaluation
  ...
  private processTurnAuthoritative(player: 'HOST' | 'GUEST', chosenOption: number) {
    ...
    if (isCorrect) {
      ...
      if (activeBuzzer === 'NONE') {
        // Clear only the host's token (guest clears theirs in setupProcessButton)
