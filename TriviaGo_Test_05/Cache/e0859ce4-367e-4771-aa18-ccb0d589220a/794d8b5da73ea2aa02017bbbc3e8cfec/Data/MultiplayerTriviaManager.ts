...
export class MultiplayerTriviaManager extends BaseScriptComponent {
  ...
  private sharedGameEntity: SyncEntity | null = null
  ...
  private hostBuzzerEntity: SyncEntity | null = null
  private guestBuzzerEntity: SyncEntity | null = null
  ...
  private onSessionReady() {
    ...
    this.sharedGameEntity = new SyncEntity(
      this,
    ...
    const hostBuzzerNetworkId = { customNetworkId: 'triviaHostBuzzer', networkIdType: 'Custom' } as any
    this.hostBuzzerEntity = new SyncEntity(
      this,
      new StoragePropertySet([this.hostBuzzedTimeProp]),
      this.isHost, // TRUE if this is host device, FALSE if guest device
      'Session',
      hostBuzzerNetworkId
    )

    // ── Create guest buzzer entity (ONLY guest can write to it) ────────────
    const guestBuzzerNetworkId = { customNetworkId: 'triviaGuestBuzzer', networkIdType: 'Custom' } as any
    this.guestBuzzerEntity = new SyncEntity(
      this,
      new StoragePropertySet([this.guestBuzzedTimeProp]),
      !this.isHost, // TRUE if this is guest device, FALSE if host device
      'Session',
      guestBuzzerNetworkId
    )
