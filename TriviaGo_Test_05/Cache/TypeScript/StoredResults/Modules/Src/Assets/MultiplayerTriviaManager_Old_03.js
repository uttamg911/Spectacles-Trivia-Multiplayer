...
var __selfType = requireType("./MultiplayerTriviaManager_Old_03");
...
/**
 * MultiplayerTriviaManager.ts — v2.1
 *
 * Change from v2:
 *   - Timer expiry with no buzz → roundState = 'PAUSED' instead of REVEAL_INCORRECT
 *   - PAUSED blocks evaluateBuzzerRaceAuthoritative and scheduleNextRound
 *   - Both devices show "No answer — tap any option to continue"
 *   - Any option tap while PAUSED: guest sends MSG_RESUME, host calls fetchAndSync directly
 *   - No delayed callback is ever scheduled from PAUSED, so no loop risk
 */
...
const MSG_GUEST_BUZZ = 'GUEST_BUZZ'; // "GUEST_BUZZ:timestamp:optionIndex"
const MSG_GUEST_RESUME = 'GUEST_RESUME'; // sent when guest taps any option while PAUSED
    ...
    var MultiplayerTriviaManager = _classThis = class extends _classSuper {
        ...
        onHostReceivedMessage(message) {
            ...
            if (message === MSG_GUEST_RESUME) {
                // Guest tapped while paused — load a new question immediately
        ...
        handleRoundStateChange() {
            ...
            if (state === 'REVEAL_CORRECT') {
                ...
                // Timer expired with no answer — halt everything, wait for a tap
                this.timerActive = false;
                this.nextRoundCountdownActive = false;
                this.hideAnswerFeedback();
                if (this.timerText)
                ...
                // NOTE: scheduleNextRound is deliberately NOT called here
            }
        }
        ...
        evaluateBuzzerRaceAuthoritative() {
            if (!this.isHost)
                return;
            // PAUSED blocks evaluation entirely — nothing runs until resumed
        ...
        onUpdate() {
            ...
            if (this.timerActive) {
                ...
                if (this.localTimeRemaining <= 0) {
                    ...
                    if (this.timerText)
                        this.timerText.text = '';
                    // FIX: no buzz in time → PAUSED, not REVEAL_INCORRECT
                    // Host sets state; both devices react in handleRoundStateChange
                    if (this.isHost) {
                        this.turnProcessed = true;
        ...
        checkUserAnswer(index) {
            ...
            // ── PAUSED: any tap resumes with a new question, no scoring ──────────────
...
//# sourceMappingURL=MultiplayerTriviaManager_Old_03.js.map