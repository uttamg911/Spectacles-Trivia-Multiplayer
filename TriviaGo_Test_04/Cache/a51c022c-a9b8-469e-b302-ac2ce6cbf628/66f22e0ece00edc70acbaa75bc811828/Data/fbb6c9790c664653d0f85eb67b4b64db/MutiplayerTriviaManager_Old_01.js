...
var __selfType = requireType("./MutiplayerTriviaManager_Old_01");
...
let MultiplayerTriviaManager = (() => {
    ...
    var MultiplayerTriviaManager = _classThis = class extends _classSuper {
        constructor() {
            ...
            // ── Single Persistent Question Property (Guarantees state delivery) ────────
        ...
        onSessionReady() {
            ...
            this.gameSyncEntity = new SyncEntity_1.SyncEntity(this, new StoragePropertySet_1.StoragePropertySet([this.jsonQuestionProp]), true, 'Session', gameNetworkId);
            ...
            // Listen to changes on our persistent string property
            this.jsonQuestionProp.onAnyChange.add(() => {
                const rawJson = this.jsonQuestionProp.currentValue;
                if (rawJson && rawJson.length > 0) {
                    this.log('Persistent question data updated on network');
            ...
            this.gameSyncEntity.notifyOnReady(() => {
                ...
                // If data already exists in the room when we connect, process it immediately
                const initialJson = this.jsonQuestionProp.currentValue;
                if (initialJson && initialJson.length > 0) {
        ...
        handleEdgeFunctionResponse(body) {
            try {
                ...
                const stringifiedPayload = JSON.stringify(r);
                // Commit the single JSON packet to the network state.
                this.jsonQuestionProp.setPendingValue(stringifiedPayload);
                // Host updates layout immediately
        ...
        parseAndApplyJson(jsonStr) {
            try {
                ...
                this.applyQuestionDataDirectly(record);
            }
            catch (e) {
                this.log(`Error parsing json string property: ${e}`);
            }
        }
...
//# sourceMappingURL=MutiplayerTriviaManager_Old_01.js.map