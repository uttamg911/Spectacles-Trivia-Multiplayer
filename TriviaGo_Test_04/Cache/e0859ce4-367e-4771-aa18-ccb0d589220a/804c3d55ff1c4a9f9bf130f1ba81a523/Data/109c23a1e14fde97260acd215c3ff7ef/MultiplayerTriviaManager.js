 ...
 *
 * Fixed Buzzer Racing Mechanics:
 * - Solved the opponent lockout bug on incorrect answers.
 * - Host acts as the authoritative referee to pass turns and process scores.
 */
...
let MultiplayerTriviaManager = (() => {
    ...
    var MultiplayerTriviaManager = _classThis = class extends _classSuper {
        constructor() {
            ...
            this.localScore = 0;
            // Track if THIS specific local player has submitted an answer in the current phase
        ...
        handleBuzzerStateChange() {
            ...
            // Reset local phase submission gate whenever the turn control assignment shifts
            this.localHasAnsweredPhase = false;
        ...
        evaluateBuzzerRace() {
            ...
            const activeBuzzer = this.currentActiveBuzzerProp.currentValue;
            // ── SCENARIO A: The initial flat-out speed race
            if (activeBuzzer === 'NONE') {
                ...
                // Process whoever registered the smaller timestamp first
        ...
        processTurnAuthoritative(player, chosenOption) {
            ...
            if (isCorrect) {
                ...
                // If this was an incorrect answer during the initial race, pass to the opponent
                if (activeBuzzer === 'NONE') {
                    // Clear tokens for the next phase so historical clicks don't auto-trigger a evaluation loop
                    ...
                    // The stealing player also guessed wrong. Game over for this round.
        ...
        checkUserAnswer(index) {
            if (!this.timerActive || this.localHasAnsweredPhase)
                return;
            if (this.roundStateProp.currentValue !== 'PLAYING')
                return;
            const currentBuzzerState = this.currentActiveBuzzerProp.currentValue;
            // Enforcement: Reject input if it is strictly the opponent's steal window
        ...
        evaluateHostQuestionFetch() {
            ...
            if (roomIsReady) {
                ...
                // Flush matching network metrics completely for the brand new round
                this.hostBuzzedTimeProp.setPendingValue('');
                this.guestBuzzedTimeProp.setPendingValue('');
