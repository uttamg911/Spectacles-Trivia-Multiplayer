...
export class MultiplayerTriviaManager extends BaseScriptComponent {
  ...
  private onSessionReady() {
    ...
    // Using direct documented SessionController method for local user string
    ...
    // 3. Dynamic Opponent Interception Subscriptions (FIXED: Using getUserId())
    session.onUserJoinedSession.add((joiningSession) => {
      if (joiningSession) {
        const oppId = joiningSession.getUserId()
        if (oppId !== this.localPlayerId) {
          this.hookOpponentStore(oppId)
        }
      }
    })

    // Pull active array records via session users wrapper (FIXED: Using getUserId())
    const currentSessions = session.getUsers()
    if (currentSessions) {
      for (let i = 0; i < currentSessions.length; i++) {
        const s = currentSessions[i]
        if (s) {
          const oppId = s.getUserId()
          if (oppId !== this.localPlayerId) {
            this.hookOpponentStore(oppId)
            break
          }
  ...
  private createReadyCheckHooks() {
    ...
    // Replaced legacy global Script timers with the safe DelayedCallback runtime event interface
