...
let MultiplayerTriviaManager = (() => {
    ...
    var MultiplayerTriviaManager = _classThis = class extends _classSuper {
        ...
        evaluateBuzzerRaceAuthoritative() {
            if (!this.isHost)
                return;
            // Ensure the host checks incoming answers as long as the round state is active
        ...
        // ───────────────────────────────────────────────────────────────────────────
        onUpdate() {
            // FIX: Host evaluation runs constantly regardless of whether the local visual timer is paused
            if (this.isHost) {
        ...
        checkUserAnswer(index) {
            ...
            // Pass information over network authority matrix
