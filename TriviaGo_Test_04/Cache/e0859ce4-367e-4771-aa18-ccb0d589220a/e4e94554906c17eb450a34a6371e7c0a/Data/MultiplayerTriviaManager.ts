 ...
 *
 * Final Fixed Role-Arbitration, Scoreboard Mirroring, and Timer Synchronization.
 */
...
export class MultiplayerTriviaManager extends BaseScriptComponent {
  ...
  private onSessionReady() {
    ...
    // FIX 1: Explicitly determine true room host role using the Session API, not SyncEntity guesswork
    ...
    this.gameSyncEntity = new SyncEntity(
      ...
      this.isHost, 'Session', gameNetworkId
    )
  ...
  // ───────────────────────────────────────────────────────────────────────────
  // FIX 2: Bulletproof Score Mirroring Cross-Talk
  // ───────────────────────────────────────────────────────────────────────────
  ...
  private evaluateHostQuestionFetch() {
    ...
    // The host can fetch if solo, or if the connected guest is also ready
    const session = SessionController.getInstance() as any
    const connectedUsers = session.getUsers?.() || session.getConnectedUsers?.() || []
    const isSolo = connectedUsers.length <= 1

    if (hostReady && (isSolo || guestReady)) {
      this.log('Conditions met. Clearing ready flags and pulling new question...')
  ...
  private handleEdgeFunctionResponse(body: string) {
    try {
      ...
      // Host writes variables out to the shared environment topology
  ...
  // ─── UI Rendering Application Blocks ───────────────────────────────────────
