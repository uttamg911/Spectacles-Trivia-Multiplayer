 ...
 *
 * Full Production Script - Fixed Score Sync via Shared Storage Property Set.
 */
...
let MultiplayerTriviaManager = (() => {
    ...
    var MultiplayerTriviaManager = _classThis = class extends _classSuper {
        constructor() {
            ...
            // ── Synced Network Properties (Bound directly to Shared Storage) ────────────
        ...
        onSessionReady() {
            ...
            // FIX: Register properties directly into the lobby's Shared Storage Property Set.
            // This bypasses Custom ID constraints and opens true read/write authority for everyone.
            const sharedPropSet = session.getSharedStoragePropertySet?.() || session.getStoragePropertySet?.();
            if (sharedPropSet) {
                sharedPropSet.getPropertyCollection().add(this.jsonQuestionProp);
                sharedPropSet.getPropertyCollection().add(this.timerStartTokenProp);
                sharedPropSet.getPropertyCollection().add(this.hostScoreProp);
                sharedPropSet.getPropertyCollection().add(this.hostReadyProp);
                sharedPropSet.getPropertyCollection().add(this.guestScoreProp);
                sharedPropSet.getPropertyCollection().add(this.guestReadyProp);
            }
            ...
            // Trigger initial load if data exists
        ...
        evaluateHostQuestionFetch() {
            ...
            if (roomIsReady) {
                this.log('All players ready. Resetting toggles and calling database...');
        ...
        syncScoreToNetwork() {
            this.log(`Syncing local score to network: ${this.localScore} (isHost: ${this.isHost})`);
            if (this.isHost) {
