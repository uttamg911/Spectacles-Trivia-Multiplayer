 ...
 *
 * Full Production Script - Fixed Guest-to-Host Sync via Realtime Store Mesh Map.
 */
...
let MultiplayerTriviaManager = (() => {
    ...
    var MultiplayerTriviaManager = _classThis = class extends _classSuper {
        ...
        onSessionReady() {
            ...
            const globalId = { customNetworkId: 'triviaGlobalRoomChannel', networkIdType: 'Custom' };
            ...
            // 2. FIX: Initialize Realtime Store mesh engine to allow multi-user updates
            ...
            if (this.realtimeStore) {
                ...
                this.realtimeStore.onKeyChanged?.add((key, value) => {
                    this.handleRealtimeStoreChange(key, value);
                });
            }
            else {
                this.log('Warning: Realtime/Ephemeral Store API missing or unreachable.');
            }
        ...
        handleRealtimeStoreChange(key, value) {
            if (!this.opponentPlayerId) {
                this.discoverPlayers();
            }
            // Parse the value string payload (format: "score:ready")
        ...
        evaluateMeshReadyStates() {
            ...
            if (this.opponentPlayerId) {
                ...
                guestReady = remoteValue.split(':')[1] === 'true';
            }
        ...
        setupProcessButton() {
            ...
            this.processButton.onTriggerUp.add(() => {
                this.log('Process button pressed. Publishing local ready status to mesh store...');
                this.hideAnswerFeedback();
                ...
                // Safely register our local ready state over open channel architecture
        ...
        syncScoreToNetwork() {
            ...
            // Write directly into the open Realtime Store mesh map using our unique user ID
            this.realtimeStore?.set?.(this.localPlayerId, `${this.localScore}:false`);
            if (this.myScoreValueText)
