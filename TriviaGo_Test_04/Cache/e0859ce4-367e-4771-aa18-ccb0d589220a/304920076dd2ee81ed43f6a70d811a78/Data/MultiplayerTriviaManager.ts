...
import { SyncEntity, NetworkIdOptions, NetworkIdType } from 'SpectaclesSyncKit.lspkg/Core/SyncEntity'
...
export class MultiplayerTriviaManager extends BaseScriptComponent {
  ...
  private onSessionReady() {
    ...
    // FIXED: Using correct import alignment and customNetworkId property
  ...
  private setupProcessButton() {
    if (!this.processButton || !this.processButton.interactable) return

    this.processButton.interactable.onTriggerUp.add(() => {
      this.hideAnswerFeedback()
  ...
  private setupAnswerButtons() {
    this.optionButton1?.interactable?.onTriggerUp.add(() => this.checkUserAnswer(1))
    this.optionButton2?.interactable?.onTriggerUp.add(() => this.checkUserAnswer(2))
    this.optionButton3?.interactable?.onTriggerUp.add(() => this.checkUserAnswer(3))
    this.optionButton4?.interactable?.onTriggerUp.add(() => this.checkUserAnswer(4))
  }
