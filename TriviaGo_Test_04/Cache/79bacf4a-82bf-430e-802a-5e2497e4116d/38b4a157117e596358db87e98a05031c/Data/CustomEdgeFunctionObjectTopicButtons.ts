...
export class CustomEdgeFunctionObjectTopicUI extends BaseScriptComponent {
  ...
  private correctAnswerPoints: number = 10
  ...
  private checkUserAnswer(index: number) {
    let correctAnswerSelected: boolean = false
    ...
    if (this.isAnswerCorrect) {
      ...
      this.incorrectText.enabled = false      
      if (this.doOnce) {
        this.scoreValue += this.correctAnswerPoints
        this.doOnce = false
      }
      this.scoreValueText.text = String(this.scoreValue)
      //doOnce = true
    }      
    else {
      this.incorrectText.enabled = true
      this.correctText.enabled = false
      if (this.doOnce && correctAnswerSelected) {
        this.scoreValue -= this.correctAnswerPoints
        this.doOnce = false
      }
