...
exports.LeaderboardExample = void 0;
var __selfType = requireType("./LeaderboardExample");
...
let LeaderboardExample = (() => {
    ...
    var LeaderboardExample = _classThis = class extends _classSuper {
        constructor() {
            super();
            ...
            this.initialPlaceholderCount = this.initialPlaceholderCount;
        ...
        onAwake() {
            // Initialize logger
            this.logger = new Logger_1.Logger("LeaderboardExample", this.enableLogging || this.enableLoggingLifecycle, true);
         ...
         */
        createLeaderboardButtons(count) {
            ...
            for (let i = 0; i < count; i++) {
                ...
                const button = buttonObj.getComponent(RectangleButton_1.RectangleButton.getTypeName());
                if (button) {
         ...
         */
        updateScrollWindowDimensions(entryCount) {
            if (!this.scrollWindow || !this.gridLayout) {
                this.logger.debug("[LeaderboardExample] ERROR: ScrollWindow or GridLayout is null!");
                return;
            }
            // Calculate total height to EXACTLY match how GridLayout lays out cells.
            // GridLayout advances by totalCellSize.y per row, which is:
            //   cellSize.y + cellPadding.y (top) + cellPadding.w (bottom)
            // Previously this only added cellPadding.y (top), dropping the bottom
            // padding for every row. That made scrollDimensions.y shorter than the real
            // content, so the ScrollWindow clamped/sprang back before the last entries
            // could be reached — the bug scaled with the number of items.
            const itemHeight = this.gridLayout.cellSize.y;
            const gap = this.gridLayout.cellPadding.y; // Top padding = gap between items (logging only)
            const totalHeight = entryCount * this.gridLayout.totalCellSize.y;
            // Get current values
            ...
            //
            // Keep the grid CENTERED within the scroller (local Y = 0).
            //
            // The ScrollWindow clamps the scroller symmetrically to
            //   ±(scrollDimensions.y - windowSize.y) / 2
            // so the only way BOTH the first and last entries can reach the window
            // edges is for the content to be centered. The previous code instead
            // offset the grid object by (windowSize.y - totalHeight)/2 AND rested at
            // scrollPosition = 0 (the MIDDLE of the clamp range). That consumed one
            // whole clamp extreme just to align the top at rest, leaving the bottom of
            // the list beyond the reachable scroll range — so the last item could never
            // fully scroll into view and stayed clipped at the end.
            const gridYPosition = 0;
            ...
            // Start scrolled to the TOP of the list. With the grid centered, the top
            // extreme of the scroll range is (windowSize.y - totalHeight) / 2.
            const topScrollPosition = (windowSize.y - totalHeight) / 2;
            this.scrollWindow.scrollPosition = new vec2(0, topScrollPosition);
            ...
            this.logger.debug(`[LeaderboardExample] Grid centered at local Y=${gridYPosition}; top scroll position=${topScrollPosition} = (windowSize.y=${windowSize.y} - totalHeight=${totalHeight}) / 2`);
         ...
         */
        updateButtonContent(buttonObj, entry) {
         ...
         */
        updateButtonText(parent, fieldName, textValue, color) {
         ...
         */
        loadLeaderboard(leaderboardName, callback) {
            if (!leaderboardName) {
            ...
            this.leaderboardModule.getLeaderboard(leaderboardCreateOptions, (leaderboardInstance) => {
         ...
         */
        createLeaderboard(generateNewName = true, callback) {
            if (generateNewName) {
                this.lastSubmittedScore = 0;
                this.leaderboardEntries = [];
        ...
        submitScore() {
            ...
            this.currentLeaderboardInstance.submitScore(scoreValue, this.submitScoreSuccessCallback.bind(this), (status) => {
        ...
        getLeaderboard() {
            ...
            this.currentLeaderboardInstance.getLeaderboardInfo(retrievalOptions, (otherRecords, currentUserRecord) => {
        ...
        submitScoreSuccessCallback(currentUserInfo) {
         ...
         */
        processLeaderboardData(otherRecords, currentUserRecord) {
            this.leaderboardEntries = [];
    ...
    __setFunctionName(_classThis, "LeaderboardExample");
    (() => {
        const _metadata = typeof Symbol === "function" && Symbol.metadata ? Object.create(_classSuper[Symbol.metadata] ?? null) : void 0;
        __esDecorate(null, _classDescriptor = { value: _classThis }, _classDecorators, { kind: "class", name: _classThis.name, metadata: _metadata }, null, _classExtraInitializers);
        LeaderboardExample = _classThis = _classDescriptor.value;
        if (_metadata) Object.defineProperty(_classThis, Symbol.metadata, { enumerable: true, configurable: true, writable: true, value: _metadata });
        __runInitializers(_classThis, _classExtraInitializers);
    })();
    return LeaderboardExample = _classThis;
})();
exports.LeaderboardExample = LeaderboardExample;
//# sourceMappingURL=LeaderboardExample.js.map